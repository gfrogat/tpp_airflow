...
_data_root = Path("/publicdata/tpp/ChEMBL")
...
df = spark.read.parquet((_data_root / "chembl_25/chembl_25_assays_dump.parquet").as_posix().repartition(100)
    ...
    if activity_comment in inactive_comments:
        ...
        if (
            ...
            return 11  # inactive (weakly)
                ...
                10 ** (9.0 - 5.5) < standard_value < 10 ** (9.0 - 4.5)
        ):
...
activity_df.write.parquet(
    (_data_root / "chembl_25/chembl_25_activity_protocol.parquet").as_posix()
)
