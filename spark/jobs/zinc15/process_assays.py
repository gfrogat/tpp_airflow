...
triples = triples \
    .withColumn("activity", F.when(F.col("avg_affinity") > 8, 1).otherwise(-1))
...
triples = triples \
    ...
    .filter((F.col("actives") > 10) & (F.col("inactives") > 10) & (F.col("actives") + F.col("inactives") > 25) \
